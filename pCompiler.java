...
public class pCompiler {
  ...
  * Run the program with an empty argument list to see possible arguments.
  *
  ...
  */
  public static void main(String[] args) {
    ...
    */
    if ((args.length <1) || (args.length > 3)) {
      System.out.println("Usage: pCompiler [-Z80] [-b] source.p");
      System.exit(1);
    }
    ...
    /* get -Z80 and -b command line options from arg list */
    z80CodeGeneration = false;
    binaryCodeGeneration = false;
    if ((args.length >= 2)){
      if ("-Z80".equals(args[0])) {
        z80CodeGeneration = true;
      } else if ("-b".equals(args[0])) {
        binaryCodeGeneration = true;
      } else {
        System.out.println("Usage: pCompiler [-Z80] [-b] source.p");
        System.exit(1);
      }
    ...
    try (FileReader fr = new FileReader(fileName); BufferedReader bufr = new BufferedReader(fr);) { 
      pCompiler compiler = new pCompiler();
      compiler.compile(bufr);
    } catch (IOException e) {
      e.printStackTrace();
      System.exit(1);
    }
  ...
  /* Constants and class member variables for interpreter */
  ...
  private void error(int n) {
    ...
    switch (n) {
      ...
      case 6 : System.out.println("unknown keyword");break;
  ...
  public void compile(BufferedReader input) throws IOException {
    this.input = input;
    try {
      init();
      prog();
      ...
      if (!errors) {
        if (z80CodeGeneration) {
          listZ80();
        } else {
          listing();
          /* initialize interpreter*/
          machineStack = new int[MAX_STACK];
          vars = new Integer[nextVariable];
          inputString = "2 3 4 5 6 7 8 0";
  ...
  private void statement(EnumSet<LexemeType> stopSet) throws IOException, FatalError {
    ...
    if (checkOrSkip(localSet, stopSet)) {
      if (lexeme.type == LexemeType.identifier) {
        ...
        debug("\noperand.opValue = " + operand.opValue);
        plantAccLoad(operand);
...
DivLoop:
  ...
  private void listing() {
    ...
    System.out.println("----------------------------------");
    for (int pos=0; pos<codePos;pos++) {
      String linePos = String.format("%3d :", pos);
      System.out.println(linePos + storeInstruction[pos].toString());
    }
    System.out.println();
  };
  
  private void listZ80() {
    if (binaryCodeGeneration) {
      /* write assembled Z80 bytes */
      String outputFilename = fileName.replace(".p", ".obj");
      try {
          BufferedWriter out = new BufferedWriter(new FileWriter(outputFilename));
          for (int pos=MIN_BIN; pos<z80PosByte;pos++) {
      ...
      try {
          BufferedWriter out = new BufferedWriter(new FileWriter(outputFilename));
          for (int pos=0; pos<z80PosLine;pos++) {
  ...
  private void runError(String message) {
    ...
    System.out.println("pc=" + pc + " : " + storeInstruction[pc].toString());
    System.out.println();
      ...
      case accStore:
        switch(instr.opType) {
          case constant:
            ...
            break;
          case var:
            if ((instr.word < 0) || (instr.word >= nextVariable)) {
              runError("division by zero");
            }
            vars[instr.word] = acc;
            break;
